...
import "../periphery/FeeHandler.sol";
import "./UniswapV2FactoryUpgradeable.sol";

contract UniswapV2Router02Upgradeable is Initializable, OwnableUpgradeable, UUPSUpgradeable, IUniswapV2Router02 {
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, "TENETSWAPV2Router: INSUFFICIENT_A_AMOUNT");
        require(amountB >= amountBMin, "TENETSWAPV2Router: INSUFFICIENT_B_AMOUNT");
        emit RmLiquidity(
            msg.sender,
            tokenA,
            tokenB,
            amountA,
            amountB,
            IERC20(tokenA).decimals(),
            IERC20(tokenB).decimals(),
            IERC20(tokenA).balanceOf(pair),
            IERC20(tokenB).balanceOf(pair),
            liquidity,
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (leftFee > 0) {
            TransferHelper.safeTransferETH(IUniswapV2Factory(factory).feeReceiver(), leftFee);
        }
    ...
    function swapExactTokensForTokens(
        ...
        if (path[0] == bgbToken) {
            ...
            if (fee > 0) {
                address feeRecipient = IUniswapV2Factory(factory).feeReceiver();
                if (feeRecipient != address(0)) {
                    TransferHelper.safeTransferFrom(mbToken, msg.sender, feeRecipient, fee);
                }
            }
            ...
            if (fee > 0) {
                address feeRecipient = IUniswapV2Factory(factory).feeReceiver();
                if (feeRecipient != address(0)) {
                    TransferHelper.safeTransfer(mbToken, feeRecipient, fee);
                }
            }
